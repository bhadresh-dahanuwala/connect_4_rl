...
def self_play_worker(game_idx, model_state, simulations, c_puct):
...
class Trainer:
    ...
    def run(self):
        ...
        try:
            for i in range(self.start_iteration, self.args['iterations']):
                ...
                if win_ratio >= 0.55:
                    print("New Champion!")
                    torch.save(checkpoint_state, os.path.join(self.checkpoint_dir, "best_model.pt"))
                else:
                    print("Challenger failed. Reverting to previous champion.")
                    self.model.load_state_dict(champion_state) # Load back original weights to GPU model
                    
                torch.save(checkpoint_state, os.path.join(self.checkpoint_dir, f"checkpoint_{i}.pt"))
        finally:
            self.executor.shutdown()

    def self_play(self, model_state):
        # self.model.eval() # No longer needed here
        examples = []
        # model_state = self.model.state_dict() # Passed in
        num_games = self.args['num_self_play_games']
        
        futures = [
            self.executor.submit(self_play_worker, i + 1, model_state, self.args['num_simulations'], self.args['c_puct']) 
    ...
    def train(self, examples):
        ...
        for epoch in range(epochs):
            ...
            for i in range(batch_count):
                ...
                boards = torch.tensor(np.array(boards), dtype=torch.float32).to(self.train_device)
                target_pis = torch.tensor(np.array(pis), dtype=torch.float32).to(self.train_device)
                target_vs = torch.tensor(np.array(vs), dtype=torch.float32).view(-1, 1).to(self.train_device)
            ...
            print(f"Epoch {epoch+1}/{epochs} - Loss: {total_loss/batch_count:.4f} (V: {total_loss_v/batch_count:.4f}, P: {total_loss_pi/batch_count:.4f})")
    ...
    def evaluate(self, challenger_state, champion_state):
        ...
        # challenger_state = self.model.state_dict() # Passed in
        # champion_state = champion_model.state_dict() # Passed in
